...
export default function Withdraw() {
    ...
    useEffect(() => {
        const fetchwithdraw = async (): Promise<void> => {
            try {
                const res = await axiosClient.get<WithdrawResponse>('/withdraw/all');
    ...
    const handleStatusChange = async (id: number, status: StatusType): Promise<void> => {
        setProcessingId(id);
        try {
            await axiosClient.get(`/withdraw/updatestatus/${id}`, {
                params: { status }
            });
